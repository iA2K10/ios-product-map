...
function buildInitialGraph(): { nodes: Node[]; edges: Edge[] } {
  ...
  categories.forEach(([catName, flows]) => {
    ...
    edges.push({
      ...
      sourceHandle: 'right',
      target: catId,
      style: { stroke: CATEGORY_COLORS[catName] || '#94a3b8', strokeWidth: 2, opacity: 0.18 },
    })
  })
...
function ProductMap() {
      ...
      setNodes(curr => {
        ...
        flows.forEach((flow) => {
          ...
          newEdges.push({
            ...
            source: `cat-${catName}`,
            target: flowId,
            style: { stroke: color, strokeWidth: 1.5, opacity: 0.18 },
          })
        })
  ...
  const expandAll = useCallback(() => {
    ...
    Object.entries(categories).forEach(([catName, flows]) => {
      ...
      allEdges.push({ id: `e-center-${catId}`, source: 'center', sourceHandle: 'right', target: catId, style: { stroke: color, strokeWidth: 2, opacity: 0.18 } })
      ...
      flows.forEach((flow) => {
        ...
        allEdges.push({ id: `e-${catId}-${flowId}`, source: catId, target: flowId, style: { stroke: color, strokeWidth: 1.5, opacity: 0.18 } })
        ...
        fullFlow.screens.forEach((screen, i) => {
          ...
          allEdges.push({ id: `e-${source}-${screenId}`, source, target: screenId, style: { stroke: color, strokeWidth: 1.5, opacity: 0.18 }, animated: true })
        })
      })
    })
